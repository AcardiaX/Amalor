...
private val ClassicActivityOpen: NavTransition = navGraphicsTransition(
    ...
    if (depth <= 0f) {
        ...
        alpha = if (scope.role == NavRole.Incoming) {
            ...
            if (settle != null) ((settle.elapsedMillis - OPEN_FADE_OFFSET) / CLASSIC_FADE_DURATION).coerceIn(0f, 1f)
            else ((progress - OPEN_FADE_START) / OPEN_FADE_SPAN).coerceIn(0f, 1f)
        } else 1f
    } else translationX = -coverProgress(depth) * driftPx
}
...
private val ClassicActivityClose: NavTransition = navGraphicsTransition(
    ...
    if (depth <= 0f) {
        ...
        alpha = if (scope.role == NavRole.Outgoing) {
            val settle = scope.settle
            if (settle != null) (1f - (settle.elapsedMillis - CLOSE_FADE_OFFSET) / CLASSIC_FADE_DURATION).coerceIn(0f, 1f)
            else ((progress - CLOSE_FADE_START) / CLOSE_FADE_SPAN).coerceIn(0f, 1f)
        } else 1f
    } else translationX = -coverProgress(depth) * driftPx
}
...
private val CrossActivityPredictive: NavTransition = navGraphicsTransition(
    ...
    val hugMax = (widthPx * (1f - CROSS_ACTIVITY_MIN_SCALE) / 2f - with(scope.density) { CrossActivityEdgeMargin.toPx() }).coerceAtLeast(0f)
    ...
    if (depth <= 0f) {
        ...
        if (scope.role == NavRole.Outgoing && committing && gesture != null) {
            ...
            translationY = snapTranslationToPixelEdge(crossActivityYShift(gesture, heightPx, scaleX, scope.density), scaleY, heightPx)
        } else {
            val easedProgress = shapedTopProgress(progress, gesture)
            scaleX = snapScaleToPixelExtent((CROSS_ACTIVITY_MIN_SCALE + (1f - CROSS_ACTIVITY_MIN_SCALE) * easedProgress) * bounce, widthPx)
            scaleY = scaleX
            translationX = snapTranslationToPixelEdge(if (hugs) (1f - easedProgress) * hugMax else 0f, scaleX, widthPx)
            alpha = when {
            ...
            translationY = snapTranslationToPixelEdge(crossActivityYShift(gesture, heightPx, scaleX, scope.density), scaleX, heightPx)
        }
    } else {
        ...
        if (gesture != null) {
            ...
            scaleX = snapScaleToPixelExtent((liveScale + (1f - liveScale) * post) * bounce, widthPx)
            scaleY = scaleX
        }
...
val AmalorAospNavTransition: NavTransition = navDirectionalTransition(
...
private fun bounceScale(settle: NavSettle?, gesture: NavGesture?): Float {
    ...
    val kick = (abs(settle.releaseVelocity) * 100f * (1f - CROSS_ACTIVITY_MIN_SCALE) * factor).coerceIn(floorKick, BOUNCE_MAX_KICK)
    ...
    return ((100f - (kick / omegaD) * exp(-BOUNCE_DAMPING * omega * t) * sin(omegaD * t)) / 100f).coerceAtMost(1f)
}
...
private fun crossActivityYShift(gesture: NavGesture?, height: Float, scale: Float, density: Density): Float {
    ...
    val maxShift = ((height - height * scale) / 2f - with(density) { CrossActivityEdgeMargin.toPx() }).coerceAtLeast(0f)
